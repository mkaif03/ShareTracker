# 📊 ShareTracker

ShareTracker is a full-stack stock portfolio tracking web application that allows users to manage, monitor, and analyze their share investments in one place. The project is designed to help users keep track of their stock holdings, calculate profits/losses, and visualize portfolio performance through a simple and intuitive interface.

This project was built as a learning-focused full-stack application with a clear separation between frontend and backend.

---

## 🚀 Features

- 📈 Add, update, and delete stock holdings
- 💹 Track portfolio value and profit/loss
- 📊 Visual representation of portfolio data
- 🔐 Backend API for managing portfolio data
- 🧱 Modular frontend and backend architecture

---

## 🧱 Project Structure

ShareTracker/
├── client/
├── server/
├── images/
├── .gitignore
├── package.json
└── README.md

---

## 🛠️ Tech Stack

**Frontend**
- HTML, CSS, JavaScript
- React (if applicable)

**Backend**
- Node.js
- Express.js

**Database**
- MongoDB (or update if different)

**Tools**
- Git & GitHub
- npm


## ⚙️ Installation & Setup

### Clone the repository

```bash
git clone https://github.com/mkaif03/ShareTracker.git
cd ShareTracker

### Backend setup
cd server
npm install
npm start

### Frontend setup
cd client
npm install
npm start

📌 Usage

Open the application in your browser

Add stock information such as name, quantity, and purchase price

View portfolio performance

Update or delete stock entries as needed

📜 License

This project is open-source and intended for educational use.
